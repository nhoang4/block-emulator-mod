...
func (bc *BlockChain) GetUpdateStatusTrie(txs []*core.Transaction) common.Hash {
	...
	for i, tx := range txs {
		...
		// senderIn := false
		if !tx.Relayed && (bc.Get_PartitionMap(tx.Sender) == bc.ChainConfig.ShardID || tx.HasBroker || tx.HasBridge) {
			// senderIn = true
		...
		// recipientIn := false
		if bc.Get_PartitionMap(tx.Recipient) == bc.ChainConfig.ShardID || tx.HasBroker || tx.HasBridge {
