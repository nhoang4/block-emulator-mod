...
# Machine B: launches the middle shard range. The supervisor runs on Machine A.
...
MACHINE_NAME="${MACHINE_NAME:-machine-b}"

SHARD_NUM="${SHARD_NUM:-64}"
NODES_IN_SHARD="${NODES_IN_SHARD:-10}"
SHARD_START="${SHARD_START:-$(((SHARD_NUM + 2) / 3))}"
SHARD_END="${SHARD_END:-$(((2 * SHARD_NUM + 2) / 3 - 1))}"
