...
# Local Linux runner for BrokerChain.
...
export CONSENSUS_METHOD=2
export BRIDGE_OVERLAY_ENABLED=0
export RUN_ROOT="${RUN_ROOT:-$REPO_ROOT/run-linux-broker-${SHARD_NUM}x${NODES_IN_SHARD}-$(date +%Y%m%d-%H%M%S)}"

echo "[broker] ConsensusMethod=2"
